...
If not using JSX with React or Vue, custom `jsxFactory` and `jsxFragment` can be configured using the [`esbuild` option](../config/shared-options#esbuild). For example for Preact:
...
`import.meta.glob` also supports importing files as strings (similar to [Importing Asset as String](./assets#importing-asset-as-string)) with the [Import Reflection](https://github.com/tc39/proposal-import-reflection) syntax:
