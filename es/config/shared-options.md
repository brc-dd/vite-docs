...
- **Tipo:** `ESBuildOptions | false`
  ...
  Cuando [`build.minify`](./build-options#build-minify) es `true`, todas las optimizaciones de minify se aplican de manera predeterminada. Para deshabilitar [ciertos aspectos](https://esbuild.github.io/api/#minify), configura cualquiera de las opciones `esbuild.minifyIdentifiers`, `esbuild.minifySyntax` o `esbuild.minifyWhitespace` en `false `. Ten en cuenta que la opción `esbuild.minify` no se puede usar para anular `build.minify`.
...
- **Por defecto:** `'spa'`
  ...
  Si tu aplicación es una aplicación de página única (SPA), una [aplicación multipáginas (MPA)](../guide/build#aplicacion-multipaginas) o una aplicación personalizada (SSR y marcos con manejo de HTML personalizado):
