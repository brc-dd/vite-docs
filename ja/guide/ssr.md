...
Vite の機能がトランスパイルされてない状態で使われている場合のように、Vite のパイプラインによって変換される必要のある依存関係は、[`ssr.noExternal`](../config/ssr-options#ssr-noexternal) に追加できます。
...
コマンドラインコマンドの `$ vite dev` と `$ vite preview` は SSR アプリでも使用することができます。開発サーバには [`configureServer`](./api-plugin#configureserver) 、プレビューサーバには [`configurePreviewServer`](./api-plugin#configurepreviewserver) で SSR ミドルウェアを追加できます。
